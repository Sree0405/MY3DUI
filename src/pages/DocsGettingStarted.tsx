...
import { gettingStartedPage } from "@/data/docs-content";
    ...
    <div className="space-y-4">
      {parts.map((part, i) => {
        if (part.startsWith("```")) {
          ...
          if (match) {
            return <CodeBlock key={i} code={match[2].trim()} language={match[1] || "bash"} />;
          }
        }
        return part.trim() ? (
          <div key={i} className="text-muted-foreground leading-relaxed whitespace-pre-line text-sm">
            {part.split("\n").map((line, li) => {
              if (line.startsWith("## ")) return <h3 key={li} className="text-lg font-bold text-foreground mt-4 mb-2">{line.slice(3)}</h3>;
